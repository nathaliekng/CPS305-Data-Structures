def precedence(oper):
    if oper in ['+', '-']:
        return 1
    elif oper in ['!']: #if the operation is a factorial, it has the highest precedence 
        return 3
    else:
        return 2
...
def operatorp(x):
    #add factorial into list of operators
    return x in ['+', '-', '/', '*', '!']
...
def parseHelper(expr, operators, operands):
    ...
    #if the value at index is a list, then it will run parseHelper again with that list 
